	...
	for (INTPTR i = 0; i < iWord; i++) {
		...
		for (INTPTR j = 0; j < jSyl; j++) {
			...
			if (S.Syl.GetLength() > 0) {
				TWA[i].TSA[j].a1 = 0; //S.Stress;
				TWA[i].TSA[j].a2 = 0; //S.DoQ;
				TWA[i].TSA[j].a3 = 0; //S.TPA.GetSize();
			} else {
				TWA[i].TSA[j].a1 = 0;
				TWA[i].TSA[j].a2 = 0;
				TWA[i].TSA[j].a3 = 0;
			}
			TWA[i].TSA[j].b1 = 0; //TWA[i].TSA[j].Stress;
			TWA[i].TSA[j].b2 = 0; //TWA[i].TSA[j].DoQ;
			TWA[i].TSA[j].b4 = j + 1;
			...
			if (S.Syl.GetLength() > 0) {
				TWA[i].TSA[j].c1 = 0; //S.Stress;
				TWA[i].TSA[j].c2 = 0; //S.DoQ;
				TWA[i].TSA[j].c3 = 0; //S.TPA.GetSize();
			} else {
				TWA[i].TSA[j].c1 = 0;
				TWA[i].TSA[j].c2 = 0;
				TWA[i].TSA[j].c3 = 0;
    ...
    for(INTPTR i = 0; i < TWA.GetSize(); i++) {
        ...
        for (INTPTR j = 0; j < TWA[i].TSA.GetSize(); j++){
            ...
            for (INTPTR k = 0; k < TWA[i].TSA[j].TPA.GetSize(); k++) {
                ...
                P.PL += L"/B:" + its(S.b1) + L"-" + its(S.b2) + L"-" + its(0) + L"@" + its(S.b4) + L"-" + its(S.b5) + L"&" + its(S.b6) + L"-" + its(S.b7)+ L"|" + S.b16;
